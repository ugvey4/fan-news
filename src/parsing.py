...
def help_info(root):
    help_label = ttk.Label(root, text= 'help', font= ('Arial', 10), 
    background=  '#2f2f2f', foreground= 'white')
    help_label.place(x = 670, y = 40)
...
def choice_sport(root):
    ...
    leagues = {
        ...
        'Серия А': {"Ювентус", "Интер", "Милан", "Наполи", "Лацио"},
        'РПЛ': {"Зенит", "ЦСКА", "Спартак Москва", "Динамо Москва", "Краснадар", "Локомотив", "Динамо Махачкала"}}
    ...
    help_button = Button(root,  text= 'help', command= help_info)
...
def menu_run(root):
    ...
    file_menu.add_command(label="Выйти", command= lambda: restart(root))

    table_menu = Menu(background='#2f2f2f', foreground= 'white', font= ('Arial', 10))
    table_menu.add_command(label= "Общие насторойки")
    table_menu.add_command(label= "Help")

    main_menu.add_cascade(label="Мой аккаунт", menu=file_menu)
    main_menu.add_cascade(label="Статистика")
    main_menu.add_cascade(label="Настройки", menu= table_menu)
